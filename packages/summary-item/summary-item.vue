<template lang="html">
  <a class="peony-summary-item" :class="['is-type-' + type]" :href="href" :target="target" :style="allWidthStyle">
    <p class="peony-summary-item__desc" v-if="$slots.describe || describe">
      <slot name="describe" v-if="$slots.describe !== undefined"></slot>
      <template v-else> {{describe}} </template>
    </p>
    <div class="peony-summary-item__title" v-if="$slots.title || title">
